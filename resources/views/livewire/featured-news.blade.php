<div class="bg-white dark:bg-gray-950 py-12 sm:py-16">
    <div class="mx-auto lg:px-0 px-4 max-w-7xl">
        <div class="mx-auto">
            <h2 class="text-4xl lg:pb-0 pb-5 font-medium tracking-tight text-gray-900 dark:text-white sm:text-3xl">Trending</h2>
        </div>
        <div class="mx-auto grid max-w-2xl auto-rows-fr grid-cols-1 gap-8 mt-6 lg:mx-0 lg:max-w-none lg:grid-cols-2">
            @for($i = 0; $i < 6; $i++)
        @php
            $duration = 1500 + ($i * 400);
            $icons = ['/video.svg', '/blog.svg', '/gallery.svg'];
            $icon = $icons[$i % count($icons)];
        @endphp
            <article class="relative isolate flex flex-col justify-end overflow-hidden rounded-2xl bg-gray-900 px-4 pb-8 pt-44 sm:pt-60 lg:pt-56 h-96">
                ...
                <img src="{{ $icon }}" class="absolute text-yellow top-2/3 mb-8 left-1/2 transform -translate-x-1/2 -translate-y-28 h-12 w-12 fill-white" alt="">
                ...
                <div class="flex mt-3 flex-wrap items-center gap-y-1 overflow-hidden text-base leading-6 text-gray-300">
                    ...
                    <div class="-ml-4 flex items-center gap-x-2">
                        <div class="flex text-white font-normal gap-x-2">
                            Test
                        </div>
                    </div>
                </div>
